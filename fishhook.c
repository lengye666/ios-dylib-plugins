// Minimal fishhook implementation for rebinding C function symbols
// Based on facebook/fishhook (BSD License)
// Only handles __la_symbol_ptr (lazy symbol pointers) on ARM64
...
static void rebind_symbols_for_image(struct rebindings_entry *rebindings,
    ...
    for (uint i = 0; i < header->ncmds; i++,
         cur += ((struct load_command *)cur)->cmdsize) {
    ...
    for (uint i = 0; i < header->ncmds; i++,
        ...
        if (strcmp(cur_seg_cmd->segname, SEG_DATA) != 0) continue;
        section_t **section_ptr = (section_t **)(cur + sizeof(segment_command_t));
        for (uint j = 0; j < cur_seg_cmd->nsects; j++) {
            section_t *sect = *section_ptr;
